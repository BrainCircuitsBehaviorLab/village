...
class TimeUtils:
    ...
    def sync(self, wall: datetime.datetime | None = None) -> None:
        self._base_wall = wall or datetime.datetime.now()
...
class Timer:
    def __init__(self, seconds: int) -> None:
        ...
        self.init_time = now_m - self.seconds
        self.ten_seconds_time = now_m - 10.0

    def has_elapsed(self) -> bool:
        now_m = time.monotonic()
        if now_m - self.init_time >= self.seconds:
            self.init_time = now_m
            self.ten_seconds_time = now_m
            return True
        return False

    def ten_seconds_elapsed(self) -> bool:
        now_m = time.monotonic()
        if now_m - self.ten_seconds_time >= 10.0:
            self.ten_seconds_time = now_m
            return True
        return False
...
class TimestampTracker:
    def __init__(self, hours: int) -> None:
        self.timestamps: list[float] = [time.monotonic()]
    ...
    def trigger_empty(self) -> bool:
        cutoff = time.monotonic() - self.hours * 3600.0
