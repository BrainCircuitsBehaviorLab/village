...
class TaskBase:
    ...
    #
    # If you are using Bpod, event registration happens automatically with the
    # correct timestamps, so you never need to call any of the 4 methods above.
    #
    # If you are using Arduino or any other controller, use the methods above to
    # register events with the correct timestamps:
    # - raspberry_timestamp: the Raspberry Pi's own clock. Obtain it with
    #   time_utils.now_timestamp().
    # - controller_timestamp: read from the controller's own clock. For example,
    #   on an Arduino you can use the millis() function, then convert to seconds
    #   by dividing by 1000.
    #
    # If you are not using a controller (i.e. the controller is the Raspberry Pi
    # itself), controller_timestamp is exactly the same as raspberry_timestamp.
    ...
    def register_start_trial(
        ...
        If you are not using a controller, (i.e. the controller is the same raspberry),
        the controller_timestamp is exactly the same as the raspberry timestamp.
        To obtain the raspberry_timestamp, use time_utils.now_timestamp().
    ...
    def register_controller_event(self, name: str, controller_timestamp: float) -> None:
        ...
        If you are not using a controller, (i.e. the controller is the same raspberry),
        this method does exactly the same as register_raspberry_event().
    ...
    # RASPBERRY EVENTS:
    #
    # register_raspberry_event
    #
    # An event generated by the Raspberry Pi itself rather than by the controller
    # — for example, camera detection or a touchscreen touch can raise events.
    ...
    def register_raspberry_event(self, name: str, raspberry_timestamp: float) -> None:
        ...
        You may want to call this for events that are generated asynchronously from the
        controller state machine, for example when executing direct functions or
        when using a camera to detect or trigger events.
        Pass time_utils.now_timestamp() as raspberry_timestamp.
    ...
    # register_value
    #
    # A value you want saved alongside the trial data — typically registered from
    # after_trial, e.g. the stimulus type, the trial outcome, etc.
    ...
    # execute_function
    #
    # Executes a registered function by its index (1-99).
