...
def get_led_strip(spi_device: str = '/dev/spidev0.0', num_leds: int = 10,
    ...
    """
    try:
        led_strip = Pi5Neo(spi_device=spi_device, num_leds=num_leds,
                           spi_speed_khz=spi_speed_khz,
                           quiet_mode=quiet_mode)
        log.info("LED strip initialized successfully.")
        return led_strip
    except Exception as e:
        log.error(f"Error initializing LED strip: {e}",
