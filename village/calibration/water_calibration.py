...
class WaterCalibration(CalibrationBase):
    ...
    def draw(self) -> None:
        ...
        self.iterations_line_edit = self.layout.create_and_add_line_edit(
            "100", 9, 19, 8, 2, self.calibration_changed
        )
        self.layout.create_and_add_label(
            "ITERATIONS",
            29,
        ...
        self.iterations_line_edit2 = self.layout.create_and_add_line_edit(
            "100", 31, 19, 8, 2, self.test_changed
        )
        ...
        for i in range(8):
            button = self.layout.create_and_add_button(
                ...
                "The value of this test is correct. Save the test result.",
                "powderblue",
            )
        ...
        for i in range(8):
            button = self.layout.create_and_add_button(
                ...
                "The error is too large. Use the test result as a new"
                " calibration point.",
                "lightcoral",
            )
            button.setDisabled(True)
        ...
        self.save_button = self.layout.create_and_add_button(
            ...
            "Save the calibration. At least two points are needed per port.",
            "powderblue",
        )
        ...
        # info layout (scrollable calibration points list)
        widget = QWidget()
    ...
    def test_button_clicked(self) -> None:
        ...
        for i in range(len(self.water_expected2)):
            if i not in self.indices2:
                ...
                try:
                    self.times2[i] = self.get_valve_time(i + 1, self.water_expected2[i])
                    ok += 1
                except Exception:
        ...
        if errors:
            ...
            text += (
                ...
                " you entered is within the calibrated range."
            )
    ...
    def save_button_clicked(self) -> None:
        counts = self.session_df["port_number"].value_counts()
        valid_ports = counts[counts >= 2].index
        removed_ports = counts[counts < 2].index.tolist()
        filtered_df = self.session_df[self.session_df["port_number"].isin(valid_ports)]
        ...
        if removed_ports:
            text = (
                "The following ports had only one calibration point and were "
                "therefore removed from the calibration: "
                + ", ".join([str(p) for p in removed_ports])
            )
            QMessageBox.information(self.window, "Warning", text)
        else:
            QMessageBox.information(
    ...
    def all_tests_done(self) -> None:
        ...
        self.session_df = pd.DataFrame(self.test_row_dicts)
